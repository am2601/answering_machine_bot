...
#     '--config',
#     type=str,
#     default='/medium-cpu.cfg',
#     help="Path to the config"
# )
# args = arg_parser.parse_args()
# config_path = args.config
config = parse_config('/medium-cpu.cfg')
...
ranker_dict = build_ranker_dict(device=device, **prior_ranker_weights, **cond_ranker_weights)
    # Run the chatbot
logger.info("Running the POST/GET bot...")
...
while True:
  ...
  f = ''
  # prompt = input('User:')
  r = requests.get("http://artemmarkovskyi.pythonanywhere.com/get_user_text")
  if r.text == 'NOTEXT':
    continue
  prompt = r.text
  print(f"text from user: {prompt}")
  if max_turns_history == 0:
      turns = []
              # elif prompt.lower() == '/start':
              #     turns = []
              #     return "Bot: Just start texting me. If I'm getting annoying, type /reset. To quit the chat, press Ctrl-C."
  elif prompt == '/reset':
      requests.post(url=f"http://artemmarkovskyi.pythonanywhere.com/post_aswer?answer=reloading")
      turns = []
      continue
  else:
    turn = {
  ...
  # print(bot_message)
  ret = r.text + ':' + bot_message
  f = requests.post(url=f"http://artemmarkovskyi.pythonanywhere.com/post_aswer?answer={ret}")
  print(f"bot answer: {bot_message}")